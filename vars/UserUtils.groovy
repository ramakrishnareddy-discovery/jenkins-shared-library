 ...
 *
 */
def removeDanglingImages(artifactName, serverIP, userId) {
	try{
		/*
		sh """
		...
		*/
		sh """
			ssh ${userId}@${serverIP} 'docker images --no-trunc -aqf dangling=true | xargs --no-run-if-empty docker rmi && 
			docker images | grep snapshot | tr -s " " | cut -d " " -f 3 | xargs --no-run-if-empty docker rmi' 
			"""	
	} catch(error) {
		echo "${error}"
 ...
 */
def stopContainer(artifactName, serverIP, userId) {
	try{
		/*
		sh """
		...
		*/
		...
		sh """
			ssh ${userId}@${serverIP} 'docker ps --no-trunc -aqf \'name=${artifactName}\' | xargs -I {} docker stop {} &&
			docker ps --no-trunc -aqf \'name=${artifactName}\' | xargs -I {} docker rm {}' 
			"""	
	} catch(error) {
		echo "${error}"
...
def loadImage(distroDirPath, artifactName, releasedVersion, serverIP, userId) {
	   // BE CAREFUL WHILE DOING THIS. IT'S GOING TO REMOVE ALL THE **PREVIOUS** TAR(UI AND API) FILES
		// To Remove snapshot TAR Files
		try {
			sh "ssh ${userId}@${serverIP} 'ls && rm *${artifactName}*.tar ' "
		} catch(error) {
			echo "${error}"
		}
	...
	sh "scp -C ${distroDirPath}/${artifactName}.tar ${userId}@${serverIP}:/home/${userId}"
	sh "ssh ${userId}@${serverIP} 'ls && docker load -i ${artifactName}.tar' "
}

def loadImageInProd(distroDirPath, artifactName, releasedVersion, serverIP) {
	sh "scp -C ${distroDirPath}/${artifactName}-${releasedVersion}.tar ${userId}@${serverIP}:/home/${userId}"
	sh "ssh ${userId}@${serverIP} 'ls && docker load -i ${artifactName}-${releasedVersion}.tar' "
}
...
def promoteAPIToEnv(artifactName, releasedVersion, PROP_ENV, serverIP, userId) {
		sh """
				ssh ${userId}@${serverIP} 'docker run -e \'SPRING_PROFILES_ACTIVE=${PROP_ENV}\' -v /var/logs/demandplannerapi:/var/logs -d -p 8099:8090 --name ${artifactName} -t ${artifactName}:${releasedVersion}'
				"""
}
